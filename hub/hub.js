...
const config = {
  ...
  openaiLiveVoice: process.env.OPENAI_LIVE_VOICE || 'marin',
  openaiLiveInstructions: process.env.OPENAI_LIVE_INSTRUCTIONS || '',
  openaiLiveTimeout: parseInt(process.env.OPENAI_LIVE_TIMEOUT || '30', 10),
...
async function buildLiveInstructions({ conversationId, logger } = {}) {
  const configuredInstructions = String(config.openaiLiveInstructions || '').trim()
  ...
  try {
    ...
    logger?.warn?.({
      conversationId,
      error: memoryErrorMessage(err),
    }, 'Live memory context unavailable')
  }
  ...
  return [
    ...
    '- Speak concise, interruptible answers. Listen while speaking and yield when the user interrupts.',
    '- Delegate questions needing detailed reasoning to the backend. Ask for clarification when context is incomplete.',
...
registerLiveRoutes(fastify, {
  config,
  buildInstructions: buildLiveInstructions,
  async delegate(history) {
    // CLI chat providers can execute tools. Voice must never enter those paths.
    if (!['gpt', 'ollama'].includes(config.chatProvider)) {
      return 'Please use text chat for this task. Voice mode cannot run device actions or code.'
    }
    const content = 'Answer this voice conversation briefly. Do not propose or execute commands. '
      + 'Ask for clarification if the latest request is incomplete. Conversation data:\n'
      + JSON.stringify(history)
    const output = await runChatProvider(config.chatProvider, { content })
    const parsed = parseProviderContent(config.chatProvider, output)
    if (parsed.commands?.length) return 'Please use text chat for device actions or code execution.'
    return parsed.text || 'Please clarify what you would like help with.'
  },
})
