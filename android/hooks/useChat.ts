...
export function useChat() {
  ...
  const startPolling = useCallback((executionId: string, originalMessageId: string) => {
    let attempts = 0;
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (result.status === 'completed' || result.status === 'failed') {
          const isSuccess = result.status === 'completed' && result.result?.success;
          ...
          setState(prev => {
            const updatedMessages = prev.messages.map(msg =>
              msg.id === originalMessageId
                ? { ...msg, executionStatus: (isSuccess ? 'completed' : 'failed') as const }
                : msg
            );
